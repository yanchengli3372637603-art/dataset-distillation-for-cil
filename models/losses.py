...
class MahalanobisLoss(nn.Module):
    ...
    def forward(self, x, y, labels):
        ...
        # Keep the return type as Tensor even when a mini-batch contains no
        # same-class pairs. The training logger calls .detach(), and returning
        # a Python float here will crash on batches without valid pairs.
        loss = x.new_zeros(())
        pair_count = 0
        ...
        for label in unique_labels:
            ...
            cov_inv = self.sigma_inv[label.item()].to(device=x.device, dtype=x.dtype)
