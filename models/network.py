...
class ViT(VisionTransformer):
    ...
    def _resize_to_patch_embed(self, x):
        target_size = getattr(self.patch_embed, 'img_size', None)
    ...
    def forward(self, x, task_id, register_blk=-1, get_feat=False, get_cur_feat=False):
        x = self._resize_to_patch_embed(x)
        ...
        for i,blk in enumerate(self.blocks):
            if self.grad_checkpointing and not torch.jit.is_scripting():
                def _blk_forward(inp, _blk=blk, _i=i):
                    return _blk(inp, task_id, register_blk==_i, get_feat=get_feat, get_cur_feat=get_cur_feat)
                x = checkpoint_utils.checkpoint(_blk_forward, x, use_reentrant=False)
            else:
